...
import { Course, UserProgress, ScratchpadNote, CourseReview } from '../types';
import { getStoredReviewsForCourse, saveCourseReview, CourseReviewModal } from './CourseReviewModal';
...
const SCRATCHPAD_KEY = 'learnsphere_scratchpad_notes_v1';

const getStoredNotesForCourse = (courseId: string): ScratchpadNote[] => {
  try {
    const raw = localStorage.getItem(SCRATCHPAD_KEY);
    if (!raw) return [];
    const allNotes: ScratchpadNote[] = JSON.parse(raw);
...
const saveNewCourseNote = (note: ScratchpadNote): ScratchpadNote[] => {
  try {
    const raw = localStorage.getItem(SCRATCHPAD_KEY);
    const allNotes: ScratchpadNote[] = raw ? JSON.parse(raw) : [];
    const updated = [note, ...allNotes];
    localStorage.setItem(SCRATCHPAD_KEY, JSON.stringify(updated));
    return updated.filter((n) => n.courseId === note.courseId);
  } catch (e) {
    return [];
  }
};
...
export const CourseOverviewScreen: React.FC<CourseOverviewScreenProps> = ({
  ...
  // Load reviews & attached scratchpad notes
  useEffect(() => {
    if (course) {
      const loadedReviews = getStoredReviewsForCourse(course.id);
      if (loadedReviews.length === 0) {
        // Seed initial verified student reviews
        const initialReviews: CourseReview[] = [
          {
            id: `rev_seed_${course.id}_1`,
            courseId: course.id,
            userId: 'usr_peer_1',
            userName: 'Maya Lin',
            userAvatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&auto=format&fit=crop&q=80',
            rating: 5,
            reviewText: `The 5-tier depth drilldowns and audio narration in this course made complex topics incredibly easy to digest! Highly recommended for exam prep.`,
            createdAt: '2 days ago',
          },
          {
            id: `rev_seed_${course.id}_2`,
            courseId: course.id,
            userId: 'usr_peer_2',
            userName: 'David Chen',
            userAvatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&auto=format&fit=crop&q=80',
            rating: 5,
            reviewText: `Clear chapter breakdown and interactive assignments at the end of each module. Helped me score 95%+ in my final quiz.`,
      ...
      const loadedNotes = getStoredNotesForCourse(course.id);
      if (loadedNotes.length === 0) {
        // Seed an initial course study note
        const firstLesson = course.modules[0]?.lessons[0];
        const seedNote: ScratchpadNote[] = [
          {
  ...
  const handleAddQuickNote = (e: React.FormEvent) => {
    ...
    const updated = saveNewCourseNote(createdNote);
  ...
  return (
    <div className="w-full max-w-full px-4 sm:px-8 lg:px-12 space-y-8 animate-fade-in pb-16">
      ...
      <CourseReviewModal
        ...
        onSaveReview={() => {
          setReviewsList(getStoredReviewsForCourse(course.id));
        }}
      />

...
// Selector called once by AiConfiguration.tsx (memoized there via useMemo, keyed on the raw
// usage data) and passed down to both AiUsageSummary.tsx and AiUsageChart.tsx as a prop -- one
// aggregation implementation, one rounding rule, computed once per data change instead of each
// view independently re-running it on every render. Always returns all 7 AI Tasks (iterating
// AI_TASK_IDS as the base, not the entries present in `data`), so a task with zero usage in the
// selected range still shows up as a real 0, not a silent omission.
...
export const useAiUsage = (): UseAiUsageResult => {
  ...
  const { data, isLoading, error } = useAsync<AiUsageEntry[]>(
    () =>
      aiUsageService
        .getUsage(dateRange)
        .then((dtos) => dtos.map(toAiUsageEntry).filter((row): row is AiUsageEntry => row !== null)),
    [],
    [dateRange],
    (err) => (err instanceof Error ? err.message : 'Could not load AI usage data.')
  );

...
import { getWeeklyGoalHours, setWeeklyGoalHours as persistWeeklyGoalHours } from '../../services/userService';
...
export const WeeklyGoalCard: React.FC<WeeklyGoalCardProps> = ({ currentHoursLearned }) => {
  const [weeklyGoalHours, setWeeklyGoalHours] = useState<number>(() => getWeeklyGoalHours());

  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [tempGoal, setTempGoal] = useState<number>(weeklyGoalHours);

  useEffect(() => {
    persistWeeklyGoalHours(weeklyGoalHours);
  }, [weeklyGoalHours]);

...
const STUDENT_NAV_SECTIONS = [
  { id: 'weekly-goal', label: 'Weekly Goal', icon: Target },
  { id: 'adaptive-schedule', label: 'Adaptive Schedule', icon: CalendarClock },
  { id: 'my-courses', label: 'My Courses', icon: BookOpen },
  { id: 'assignments', label: 'Assignments', icon: FileCheck2 },
  { id: 'tutor-booking', label: 'Tutor Booking & Group Learning', icon: GraduationCap },
];
  ...
  return (
    <div className="space-y-8 pb-12 w-full">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-[#143358] text-white p-6 sm:p-10 shadow-xl border border-white/10">
        <div className="absolute top-0 right-0 -mt-10 -mr-10 w-80 h-80 bg-[#BA5012]/15 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="flex flex-wrap items-center gap-3">
            {enrolledCourses.length > 0 && (
              <button
                ...
                className="px-5 py-2.5 bg-[#BA5012] hover:bg-[#BA5012]/90 text-white font-bold text-xs rounded-xl shadow-lg shadow-[#BA5012]/30 flex items-center space-x-2 transition-all transform hover:-translate-y-0.5"
              >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white border border-[#E1DED4] shadow-xs flex items-center space-x-4">
          <div className="p-3 rounded-xl bg-[#BA5012]/10 text-[#BA5012] border border-[#BA5012]/20">
            <Flame className="w-6 h-6 fill-[#BA5012]" />
          </div>
          <div>
      ...
      <div className="p-6 rounded-3xl bg-white border border-[#E1DED4] shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            <div className="p-3.5 rounded-2xl bg-[#BA5012]/10 text-[#BA5012] border border-[#BA5012]/20 flex-shrink-0">
              <Flame className="w-7 h-7 fill-[#BA5012]" />
            </div>
            <div>
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-2xl border text-center transition-all flex flex-col items-center justify-between space-y-1.5 ${
                    isCompleted
                      ? 'bg-[#FAF7EC] border-[#BA5012]/40 shadow-2xs'
                      : 'bg-white border-[#E1DED4] opacity-60'
                  } ${isToday ? 'ring-2 ring-[#BA5012]' : ''}`}
                >
                  <span className="text-[10px] font-bold text-[#5E6A79] uppercase">{dayName}</span>
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center ${
                      isCompleted ? 'bg-[#BA5012] text-white shadow-xs' : 'bg-slate-100 text-slate-400'
                    }`}
                  >
                    {isCompleted ? (
      ...
      <div id="my-courses" className="scroll-mt-24 grid grid-cols-1 lg:grid-cols-3 gap-8">

        {/* Left Column (2/3): My Courses */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center space-x-2">
                ...
                <select
                  ...
                  className="px-3 py-1.5 bg-white border border-[#E1DED4] rounded-xl text-xs font-bold text-[#142030] shadow-xs focus:outline-none focus:ring-2 focus:ring-[#BA5012] cursor-pointer"
                >
              ...
              <div className="space-y-4">
                {enrolledCourses.map((course) => {
                  ...
                  return (
                    <div
                      key={course.id}
                      className="p-5 rounded-2xl bg-white border border-[#E1DED4] hover:border-[#BA5012] shadow-xs hover:shadow-xl hover:shadow-[#143358]/10 hover:-translate-y-1.5 transition-all duration-300 ease-out flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group cursor-pointer"
                    >
                        ...
                        <div className="space-y-1 min-w-0 flex-1">
                          <div className="flex items-center flex-wrap gap-2">
                            ...
                            {course.targetGradeTag && (
                              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#FAF7EC] text-[#BA5012] border border-[#BA5012]/30">
                                {course.targetGradeTag}
                              </span>
                            )}
                          ...
                          <div className="w-full max-w-md pt-2 space-y-3">
                            ...
                            <div className="pt-2 border-t border-[#E1DED4]/60 space-y-2">
                              <p className="text-[11px] font-extrabold uppercase tracking-wider text-[#143358] flex items-center space-x-1">
                                <BookOpen className="w-3.5 h-3.5 text-[#BA5012]" />
                                      ...
                                      <div className="w-full h-1.5 bg-white rounded-full overflow-hidden border border-[#E1DED4]">
                                        <div
                                          className={`h-full rounded-full transition-all duration-300 ${
                                            modPct === 100 ? 'bg-[#179765]' : 'bg-[#BA5012]'
                                          }`}
                                          style={{ width: `${modPct}%` }}
                                        />
                                      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-2xl bg-[#143358] border border-white/10 text-white shadow-md space-y-4">
            ...
            <button
              ...
              className="w-full py-2.5 bg-[#BA5012] hover:bg-[#BA5012]/90 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center justify-center space-x-2"
            >
              <span>Join Room</span>
          ...
          <div className="p-5 rounded-2xl bg-white border border-[#E1DED4] shadow-xs space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start space-x-3 p-2.5 rounded-xl bg-[#FAF7EC] border border-[#E1DED4] text-xs">
                <Award className="w-4 h-4 text-[#BA5012] mt-0.5 flex-shrink-0" />
                <div>
      ...
      {/* Assignments (merged from the former standalone Assignments tab -- PRD FR-4..FR-8) */}
      ...
      {/* Get Help: Tutor Booking & Group Learning (merged from the former Tutor Hub) --
          StudentTutorBookingView carries its own section-intro banner, so no duplicate
          heading is added here. */}
      <section id="tutor-booking" className="scroll-mt-24 pt-4 border-t border-[#E1DED4]">
        <StudentTutorBookingView
          user={user as UserProfile}
          tutorSlots={tutorSlots}
          onBookSlot={onBookSlot}
          groupRequests={groupRequests}
          onRequestGroupClass={onRequestGroupClass}

...
import { Calendar, Plus, Trash2, CheckCircle2, Clock, BookOpen, GripVertical } from 'lucide-react';
import { Course } from '../../types';
import { ScheduledLessonItem, ScheduleDay, getSchedule, saveSchedule } from '../../services/scheduleService';
...
export const AdaptiveSchedule: React.FC<AdaptiveScheduleProps> = ({
  ...
  const [selectedDayToAdd, setSelectedDayToAdd] = useState<ScheduleDay>('Mon');
  ...
  useEffect(() => {
    setScheduledItems(getSchedule(courses));
  }, [courses]);
  ...
  const handleAddLessonToSchedule = (e: React.FormEvent) => {
    ...
    if (course && lesson) {
      ...
      showToast({ message: `"${lesson.title}" added to your ${selectedDayToAdd} schedule.`, variant: 'success' });
  ...
  const handleDropOnDay = (day: ScheduleDay) => {
    ...
    <div className="w-full bg-white rounded-3xl border border-[#E1DED4] p-6 sm:p-8 shadow-xs space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#E1DED4] pb-6">
        <div className="space-y-1">
          <div className="flex items-center space-x-2 text-[#BA5012]">
      ...
      <div className="p-4 rounded-2xl bg-[#FAF7EC]/70 border border-[#E1DED4] space-y-2">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
          <span className="text-xs font-bold text-[#142030] flex items-center space-x-1.5 min-w-0">
            <GripVertical className="w-4 h-4 text-[#BA5012] shrink-0" />
            <span className="truncate">Drag & Drop Lesson Deck (Drag onto any day below):</span>
          </span>
          <span className="text-[10px] text-[#5E6A79] font-medium shrink-0">
        ...
        {DAYS_OF_WEEK.map((day) => {
          ...
          return (
            ...
            >
                      ...
                      <div
                        ...
                        className={`p-2.5 rounded-xl border text-xs transition-all space-y-1 relative group ${
                          item.completed
                            ...
                            : 'bg-white border-[#E1DED4] text-[#142030] hover:border-[#BA5012]'
                        }`}
                      >
              ...
              >
                <Plus className="w-3 h-3 text-[#BA5012]" />
                <span>Add</span>
              </button>
            </div>
          );
            ...
            <form id="schedule-lesson-form" onSubmit={handleAddLessonToSchedule} className="space-y-4">
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => setSelectedDayToAdd(e.target.value as ScheduleDay)}
                  className="w-full p-2.5 rounded-xl bg-white border border-[#E1DED4] text-xs font-bold text-[#142030] focus:outline-none focus:ring-2 focus:ring-[#BA5012]"
                >
                  {DAYS_OF_WEEK.map((d) => (

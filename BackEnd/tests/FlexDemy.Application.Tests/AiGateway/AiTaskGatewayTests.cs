...
{
    private static AiTaskConfigDto MakeConfig(string taskId = AiTaskIds.ExplainTopic) => new(
    ...
    [Fact]
    public async Task ExplainTopicAsync_threads_distinct_courseId_and_tutorId_through_to_RecordUsageAsync_without_swapping_them()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
        gateway.ExplainTopicAsync(Arg.Any<AiGatewayRequest>(), Arg.Any<CancellationToken>()).Returns(PrimaryResponse());
        ...
        await sut.ExplainTopicAsync(request);

        await usageService.Received(1).RecordUsageAsync(
            AiTaskIds.ExplainTopic, "Groq", "llama-4-maverick", Arg.Any<AiGatewayUsage>(), false, "course_42", "tutor_7", Arg.Any<CancellationToken>());
    }
    ...
    [Fact]
    public async Task ExplainTopicAsync_primary_succeeds_never_calls_fallback()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
        gateway.ExplainTopicAsync(Arg.Is<AiGatewayRequest>(r => r.Provider == "Groq"), Arg.Any<CancellationToken>()).Returns(PrimaryResponse());
        var usageService = Substitute.For<IAiUsageService>();
        var budgetService = Substitute.For<IAiBudgetService>();
        budgetService.TryReserveAsync(Arg.Any<string>(), Arg.Any<decimal>(), Arg.Any<CancellationToken>()).Returns(true);
        var sut = new AiTaskGateway(gateway, configService, usageService, budgetService, NullLogger<AiTaskGateway>.Instance);

        var result = await sut.ExplainTopicAsync(Request());

        Assert.Equal("primary content", result.Content);
        Assert.Equal("Groq", result.Provider);
        Assert.False(result.IsFallbackServed);
        await gateway.DidNotReceive().ExplainTopicAsync(Arg.Is<AiGatewayRequest>(r => r.Provider == "OpenRouter"), Arg.Any<CancellationToken>());
        await usageService.Received(1).RecordUsageAsync(
            AiTaskIds.ExplainTopic, "Groq", "llama-4-maverick", Arg.Any<AiGatewayUsage>(), false, null, null, Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task ExplainTopicAsync_primary_fails_falls_back_to_the_configured_secondary()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
        gateway.ExplainTopicAsync(Arg.Is<AiGatewayRequest>(r => r.Provider == "Groq"), Arg.Any<CancellationToken>())
            .Returns(Task.FromException<AiGatewayResponse>(new AiGatewayException("rate limited")));
        gateway.ExplainTopicAsync(Arg.Is<AiGatewayRequest>(r => r.Provider == "OpenRouter"), Arg.Any<CancellationToken>())
    ...
    [Fact]
    public async Task ExplainTopicAsync_both_primary_and_fallback_fail_throws_AiTaskUnavailableException()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
        gateway.ExplainTopicAsync(Arg.Any<AiGatewayRequest>(), Arg.Any<CancellationToken>())
        ...
        var ex = await Assert.ThrowsAsync<AiTaskUnavailableException>(() => sut.ExplainTopicAsync(Request()));
    ...
    [Fact]
    public async Task ExplainTopicAsync_a_non_AiGatewayException_from_the_primary_propagates_without_attempting_fallback()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => sut.ExplainTopicAsync(Request()));
        await gateway.Received(1).ExplainTopicAsync(Arg.Any<AiGatewayRequest>(), Arg.Any<CancellationToken>());
        ...
        await budgetService.Received(1).ReleaseReservationAsync(AiTaskIds.ExplainTopic, Arg.Any<decimal>(), Arg.Any<CancellationToken>());
    }
    ...
    [Fact]
    public async Task DescribeNotationAsync_primary_fails_falls_back_to_the_configured_secondary()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.DescribeNotation, Arg.Any<CancellationToken>()).Returns(MakeConfig(AiTaskIds.DescribeNotation));
        var gateway = Substitute.For<IAiGateway>();
        gateway.DescribeNotationAsync(Arg.Is<AiGatewayRequest>(r => r.Provider == "Groq"), Arg.Any<CancellationToken>())
            .Returns(Task.FromException<AiGatewayResponse>(new AiGatewayException("rate limited")));
        gateway.DescribeNotationAsync(Arg.Is<AiGatewayRequest>(r => r.Provider == "OpenRouter"), Arg.Any<CancellationToken>())
            .Returns(FallbackResponse());
        var usageService = Substitute.For<IAiUsageService>();
        var budgetService = Substitute.For<IAiBudgetService>();
        budgetService.TryReserveAsync(Arg.Any<string>(), Arg.Any<decimal>(), Arg.Any<CancellationToken>()).Returns(true);
        ...
        var result = await sut.DescribeNotationAsync(Request());
    ...
    [Fact]
    public async Task ExplainTopicAsync_a_RecordUsageAsync_failure_does_not_fail_the_call()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
    ...
    [Fact]
    public async Task ExplainTopicAsync_budget_exceeded_throws_before_any_gateway_call_is_attempted()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
        ...
        await Assert.ThrowsAsync<AiTaskBudgetExceededException>(() => sut.ExplainTopicAsync(Request()));

        await gateway.DidNotReceiveWithAnyArgs().ExplainTopicAsync(default!, default);
    }

    [Fact]
    public async Task ExplainTopicAsync_the_reservation_estimate_prices_the_prompt_not_just_the_completion()
    {
        var configService = Substitute.For<IAiConfigService>();
        ...
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(
        ...
        budgetService.TryReserveAsync(AiTaskIds.ExplainTopic, Arg.Do<decimal>(e => capturedEstimate = e), Arg.Any<CancellationToken>()).Returns(true);
        var sut = new AiTaskGateway(gateway, configService, usageService, budgetService, NullLogger<AiTaskGateway>.Instance);
        var longPromptRequest = new AiTaskRequest([new AiGatewayMessage("user", new string('a', 4000))], MaxTokens: 0);

        await sut.ExplainTopicAsync(longPromptRequest);
    ...
    [Fact]
    public async Task ExplainTopicAsync_a_successful_call_settles_the_budget_with_the_estimated_and_actual_costs()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
        ...
        await budgetService.Received(1).SettleAsync(AiTaskIds.ExplainTopic, capturedEstimate, 7.5m, Arg.Any<CancellationToken>());
        await budgetService.DidNotReceiveWithAnyArgs().ReleaseReservationAsync(default!, default, default);
    }

    [Fact]
    public async Task ExplainTopicAsync_both_primary_and_fallback_fail_releases_the_full_reservation_and_still_propagates_the_original_exception()
    {
        var configService = Substitute.For<IAiConfigService>();
        configService.GetTaskConfigAsync(AiTaskIds.ExplainTopic, Arg.Any<CancellationToken>()).Returns(MakeConfig());
        var gateway = Substitute.For<IAiGateway>();
        ...
        await Assert.ThrowsAsync<AiTaskUnavailableException>(() => sut.ExplainTopicAsync(Request()));

        await budgetService.Received(1).ReleaseReservationAsync(AiTaskIds.ExplainTopic, capturedEstimate, Arg.Any<CancellationToken>());

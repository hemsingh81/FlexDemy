...
{
    private sealed record Sut(ParseFileJob Job, ICourseFileRepository Repository, IUnitOfWork UnitOfWork, IFileStorageService FileStorage, IDocumentParser DocumentParser, IExtractStructureJobEnqueuer ExtractEnqueuer);
    ...
    private static Sut MakeSut()
    {
        ...
        var job = new ParseFileJob(repository, unitOfWork, fileStorage, documentParser, extractEnqueuer);
        return new Sut(job, repository, unitOfWork, fileStorage, documentParser, extractEnqueuer);
    }
    ...
    private static PerformContext MakePerformContext(int? retryCount)
    {
        ...
        var job = new Job(typeof(IParseFileJob), method, "file_1", CancellationToken.None, null!);
...
#pragma warning restore CS0618
    ...
    public async Task RunAsync_a_successful_parse_sets_Status_Extracting_and_ParsedContent_and_enqueues_the_extract_job()
    {
        ...
        sut.ExtractEnqueuer.Received(1).Enqueue("file_1");
    }
    ...
    public async Task RunAsync_a_successful_parse_whose_extract_enqueue_throws_marks_Failed_with_an_accurate_reason()
    {
        ...
        sut.DocumentParser.ParseAsync(Arg.Any<Stream>(), file.FileName, file.ContentType, Arg.Any<CancellationToken>())
            .Returns(new DocumentParseResult(true, "# Parsed Notes", null));
        sut.ExtractEnqueuer.When(e => e.Enqueue(Arg.Any<string>())).Do(_ => throw new InvalidOperationException("Hangfire storage unavailable"));
    ...
    public async Task RunAsync_DocumentParsingUnavailableException_on_a_non_final_attempt_propagates_uncaught_leaving_Status_Parsing()
    {
        ...
        await Assert.ThrowsAsync<DocumentParsingUnavailableException>(() => sut.Job.RunAsync("file_1", CancellationToken.None, context));

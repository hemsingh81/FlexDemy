...
components:
  ...
  course-content-editor:
    bodyComposition: 'As of the ContentAuthoring PRD (2026-08-16): the body is a single flowing document per Chapter (title, then content-doc-heading/content-page-marker/content-resource-block/prose blocks in reading order), not the earlier Chapter/Topic/Subtopic/ContentBlock tree -- see content-doc-heading, content-page-marker, content-resource-block, content-slash-menu above. The outline rail described below is now a Table-of-Contents auto-derived from the document''s own headings, not a separately-managed entity tree.'
    background: '#ffffff'
    ...
    transition: 'Zoom in/zoom out on every entry into a state, not an instant cut -- entering Maximized reuses `fade-in-scale` (150ms ease-out, scale 0.95->1, the same shared entrance keyframe every transient surface in this app already uses); entering/returning to Normal uses a new `zoom-out` keyframe (150ms ease-out, scale 1.05->1), its mirror-image counterpart. The two must stay genuinely distinct keyframes, not the same one reused for both states -- an unchanging class value across the toggle never restarts a CSS animation, so a shared keyframe would only ever play once, on first mount.'
  content-doc-heading:
    domModel: 'Real native heading elements, contenteditable (a real `<h1>`/`<h2>`/`<h3>` with `contenteditable="true"`, the same technique Notion/Confluence-class editors use) -- NEVER a `<textarea>` or styled `<div>` standing in for a heading. This is a correctness fix, not a style note: a heading that is also editable must stay a real heading in the accessibility tree, or a screen reader''s native heading-navigation (the "H" key) silently stops finding it.'
    levels: 'Chapter title = h1 (one per document). Topic = h2. Sub-Topic = h3. Page marker (below) = h4 -- a Page is a real navigable unit and belongs in the native heading tree too, not just the Table-of-Contents rail, so heading-navigation and the rail always reach the same set of stops.'
    pattern: 'Topic = font-display h2, Sub-topic = font-display h3, both with a small amber eyebrow tag ("TOPIC"/"SUB-TOPIC") above -- plain document typography, no card/border chrome. Supersedes content-tree-node below (ContentAuthoring PRD): Chapter/Topic/Sub-Topic are containers rendered as headings inside one flowing per-chapter document, not bordered tree rows.'
    confirmedState: '{colors.signal-green} filled check-glyph beside the heading once confirmed -- unconfirmed shows an outlined/empty circle glyph in {colors.muted}, i.e. a shape difference, not a color-only difference (see Accessibility Floor) -- matches the Page marker''s badge-pill discipline below rather than a bare color-swapped dot.'
  content-tree-node:
    status: 'Superseded by content-doc-heading + content-page-marker (ContentAuthoring PRD, 2026-08-16) -- kept here only as history for any code/spec still referencing the pre-f3131d9 tree. Do not build new UI against this entry.'
    background: '#ffffff'
    border: '1px solid {colors.hairline}'
    rounded: '{rounded.DEFAULT}'
  ...
  content-page-marker:
    background: 'none -- sits inline in the document flow'
    border-top: '1px dashed {colors.hairline}'
    domModel: 'A real `<h4>` (see content-doc-heading.levels) carrying the visual marker styling below -- included in native heading-navigation, not a styled bar with no heading role. Resolves the ToC-rail-vs-heading-tree mismatch: rail and native heading nav must always list the same stops.'
    pattern: 'The one place a Page (unit of content, ContentAuthoring PRD) gets a lightweight marker rather than pure typography: a dashed top rule, a small amber document-glyph, "PAGE" eyebrow, the page title inline, and a right-aligned Confirmed/Unconfirmed {components.badge-pill}. Everything below it until the next marker or heading is that page''s body.'
  content-resource-block:
    background: '{colors.parchment}'
    border: '1px solid {colors.hairline}'
    rounded: '{rounded.lg}'
    pattern: 'Generic, reusable "Learning Resources" section block (ContentAuthoring PRD) -- the SAME component whether attached to a Chapter, a Topic, a Sub-Topic or nested inside a Page''s body; never a fixed app-chrome sidebar. Header row: paperclip glyph + "Learning Resources" title + a small muted tag naming what it''s attached to. Body: one {components.badge-pill}-tagged resource row per file (Inline/Attachment role, muted for read-only/inherited), plus a dashed-border drop zone with Upload / Attach existing / Insert-from-file actions.'
    resourceRowControls: 'Every resource row is keyboard-operable end to end, not just addable by keyboard: a real icon-button trio per row (role select as a small `<select>` or button+menu, not a static badge; remove; reorder via move-up/move-down, same non-drag-only discipline as every other reorderable list in this spec) plus an inline-editable caption field. Inherited/read-only rows replace all three controls with one real focusable link ("Manage on Chapter") that navigates to the owning ancestor -- never plain descriptive text standing in for a link.'
  content-slash-menu:
    background: '#ffffff'
    border: '1px solid {colors.hairline}'
    rounded: '{rounded.lg}'
    shadow: 'shadow-xl -- an overlay that must clearly float above document text, same elevation tier as {components.modal}'
    triggerAffordance: 'The "+" is not hover-only chrome: a small circular icon button, `{rounded.full}`, at the start of every empty line and at the end of every block, visible on `:hover` **and** `:focus-within` of that line/block (so Tab-only navigation reveals it exactly as reliably as a mouse does) -- see EXPERIENCE.md Accessibility Floor for why this is the primary path, "/" the shortcut layered on top.'
    ariaWiring: 'Trigger field: `role="combobox"`, `aria-expanded`, `aria-controls` pointing at the menu. Menu: `role="listbox"`, each command `role="option"`; category eyebrow labels are `role="group"`/`aria-label`, skipped by Arrow-key traversal (not counted as options). Highlighted option exposed via `aria-activedescendant` on the trigger, matching content already stated below. Zero-match filter state renders a plain-text "No matching blocks" row inside the listbox (not an empty/collapsed menu) so the empty state is itself announced like every other empty state in this spec.'
    keyboardModel: 'Arrow Up/Down moves the highlighted option (Tab is NOT repurposed as a navigation key -- Tab always exits the field to the next focusable element, closing the menu without inserting, same as leaving any other combobox). Enter commits the highlighted option. Escape closes without inserting and strips the typed "/"+query text back to nothing, returning focus to the exact document position the "/" was typed at.'
    postInsertBehavior: 'On commit, focus moves into the newly inserted block''s first editable field (a heading''s title, an image block''s upload target, etc.) and an `aria-live="polite"` region announces what was inserted ("Image block inserted", "New Page added") -- the same discipline already applied to extraction status, Publishing progress, and Hold-then-Reviewed elsewhere in this spec, extended to this insertion.'
    imeAndBrowserSafety: 'The "/" keydown handler is scoped to fire only while the caret sits inside this editor''s own editable region (never a document-level/global listener), and is gated on `!event.isComposing` so it cannot fire mid-Devanagari-IME-composition -- both requirements exist specifically because a global or composition-blind "/" listener would collide with Firefox''s native Quick Find and with this product''s own first-class Hindi content support.'
    pattern: 'Opened by typing "/" on an empty line, or by clicking a persistent "+" affordance (the accessible, non-keyboard-shortcut-dependent path -- see EXPERIENCE.md Accessibility Floor). Search/filter row at top echoing the typed query with the match substring highlighted in {colors.citrus-amber}-tinted {rounded.sm} marks; below it, grouped command rows (category eyebrow label, then icon-well + label + one-line description per row), the top/keyboard-selected match highlighted with a filled icon-well and {colors.surface-secondary} row background.'
  spinner:
    pattern: 'The shared Loader2/animate-spin icon already used app-wide (Admin tables, form-submit buttons, ConfirmModal) -- xs size (12px) inline before label text, white on a filled/colored background, {colors.muted} on a light background. Not a new component, this entry exists only so `{components.spinner}` resolves as a token wherever it''s cited.'
...
- **Course Content Editor — body composition** *(ContentAuthoring PRD, 2026-08-16)* — The card shell, Normal/Maximized toggle, and lifecycle header below are unchanged. What renders inside changed: a single flowing document per Chapter (heading/page-marker/resource-block/prose blocks in reading order — see `{components.content-doc-heading}`, `{components.content-page-marker}`, `{components.content-resource-block}`, `{components.content-slash-menu}` above), not the Chapter/Topic/Subtopic/ContentBlock tree the rest of this entry describes. The outline rail is now a Table-of-Contents auto-derived from the document's headings.
- **Course Content Editor** (`{components.course-content-editor}`) *(New Course Wizard PRD — body composition below superseded by ContentAuthoring PRD, see the "body composition" bullet above; `{components.content-tree-node}` reference in the next sentence is history only)* — Full-width page surface, not a panel or modal, reached after the wizard's Step 4 — this is the one wizard-adjacent surface that behaves like Course Player's reading pane rather than like the metadata side panel, because a Chapter→Topic→Subtopic tree with rendered math/chemistry/image content needs reading and editing room a 640px blade can't give it. White shell, hairline border, `{rounded.lg}` — a real card shell now (previously flat/borderless against the page background, an inconsistency with every other top-level card in the system; fixed alongside the Maximize/Restore toggle below). *(Superseded body composition, kept as history:)* Composed from `{components.content-tree-node}` rows plus `{components.extraction-status-badge}` per uploaded file; header carries a persistent Draft→In Review→Review Confirmed→Published stage indicator (added in accessibility review, so the wizard's "Step N of 4" orientation doesn't vanish at the handoff), and **Confirm** and **Review as Student** actions live in the same header/footer area, not buried in scroll. **Maximize/Restore toggle:** defaults to Normal — a centered `max-w-4xl` card (`shadow-lg`, bordered/rounded, no sticky header — scrolls with the page like any other card), so a tutor lands with the surrounding Dashboard chrome still visible. Maximize turns it into a true full-viewport takeover — `fixed inset-0`, `z-50` (stacking above the Navbar's own `z-50` sticky header, the same idiom as this app's other fixed-inset-0 overlays: `{components.side-panel}`, `{components.modal}`), edge-to-edge with no border/rounding/shadow, header pinned at the top while the body scrolls in its own region beneath it — for a genuinely distraction-free editing session, not just a wider in-page card. Restore returns it to Normal. Always resets to Normal on a fresh open. See Do's and Don'ts for why Normal's width cap — now the default, not just a toggled-away-from state — is a deliberate exception to the `w-full` rule, not a regression of it.
- **Content tree node** (`{components.content-tree-node}`) — *Superseded, ContentAuthoring PRD (2026-08-16) — kept as history only, see `{components.content-tree-node}.status`.* Previously: one row per Chapter/Topic/Subtopic/Content Block inside Course Content Editor, expand/collapse + edit/delete/reorder controls, explicit Confirm action, indentation-nested. Do not build new UI against this description.
- **Content document — heading structure** (`{components.content-doc-heading}`) — Chapter/Topic/Sub-Topic render as **real, native, contenteditable heading elements** (h1/h2/h3 respectively — see `{components.content-doc-heading}.levels`), styled as Fraunces display type with a small amber eyebrow tag, inside one flowing per-chapter document, not bordered rows and never a styled `div`/`textarea` standing in for a heading — matches the discipline real document tools apply: headings and paragraphs are typography, only genuinely block-shaped things get card framing (see below). Confirmed state is a filled check-glyph beside the heading (unconfirmed: an outlined circle glyph) — a shape difference, never color-alone.
- **Content document — Page marker** (`{components.content-page-marker}`) — The one place in the document that gets a lightweight bar rather than pure typography, since a Page (the unit of content) carries its own title and Confirmed/Unconfirmed state: dashed top rule, document glyph, "PAGE" eyebrow, inline title, right-aligned `{components.badge-pill}`. Also a real `h4` (see `{components.content-doc-heading}.levels`) — included in native heading-navigation exactly like the Table-of-Contents rail includes it, never a rail-only stop.
- **Content document — Learning Resources block** (`{components.content-resource-block}`) — Generic "Learning Resources" section — parchment fill, `{rounded.lg}` — reusable at any level (Chapter, Topic, Sub-Topic, or nested inside a Page's body); the same component every time, never a fixed sidebar-only panel. Resource rows use `{components.badge-pill}` for Inline/Attachment role; every row's role, caption, remove and reorder controls are real keyboard-operable elements (see `{components.content-resource-block}.resourceRowControls}`), and a read-only inherited row is a real focusable link to its owning ancestor, not static muted text.
- **Content document — "/" slash-command menu** (`{components.content-slash-menu}`) — Opens on typing "/" on an empty line **or** clicking a persistent "+" affordance visible on hover *and* keyboard focus (the click path is the accessible primary entry — see EXPERIENCE.md Accessibility Floor, since a keyboard-shortcut-only trigger would be FlexDemy's first departure from its mouse/touch-first stance). White overlay, `shadow-xl` (same elevation as `{components.modal}`), a filter-echo row at top, grouped/categorized command rows below with the keyboard-highlighted match shown via a filled icon-well and `{colors.surface-secondary}` background — not color-alone. Full ARIA combobox/listbox wiring, keyboard model, zero-match state, post-insert focus/announcement, and IME/browser-shortcut safety are specified on the token itself (`ariaWiring`, `keyboardModel`, `postInsertBehavior`, `imeAndBrowserSafety` above) — this is the product's first command-driven interaction and is specified to the same rigor as its modal/combobox-adjacent siblings, not left to implementation judgment.
